...
public interface WxPayService {
   ...
   *
   * @param request 请求对象，注意一些参数如appid、mchid等不用设置，方法内会自动从配置对象中获取到（前提是对应配置中已经设置）
   * @deprecated 建议使用 {@link com.github.binarywang.wxvehicle.service.WxPayService#createOrder(WxPayUnifiedOrderRequest)}
   */
  @Deprecated
   ...
   * <pre>
   * 扫码支付模式二生成二维码的方法.
   * 对应链接格式：weixin：//wxvehicle/bizpayurl?sr=XXXXX。请商户调用第三方库将code_url生成二维码图片。
   ...
   *  应用场景：
   *     该接口主要用于扫码原生支付模式一中的二维码链接转成短链接(weixin://wxvehicle/s/XXXXXX)，减小二维码数据量，提升扫描速度和精确度。
