 ...
 */
public class WxProfitSharingConfig {
   ...
   * 签名方式
   * 有两种HMAC_SHA256 和MD5
   * @see WxProfitSharingConstants.SignType
   */
  ...
  public SSLContext initSSLContext() throws WxProfitSharingException {
    if (StringUtils.isBlank(this.getMchId())) {
      throw new WxProfitSharingException("请确保商户号mchId已设置");
    }

    if (StringUtils.isBlank(this.getKeyPath())) {
      throw new WxProfitSharingException("请确保证书文件地址keyPath已配置");
    }
    ...
    if (this.getKeyPath().startsWith(prefix)) {
      ...
      inputStream = WxProfitSharingConfig.class.getResourceAsStream(path);
      if (inputStream == null) {
        throw new WxProfitSharingException(fileNotFoundMsg);
      }
    } else {
      try {
        ...
        throw new WxProfitSharingException(fileHasProblemMsg, e);
      }
    }
