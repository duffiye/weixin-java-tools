 ...
 */
public class WxProfitSharingServiceJoddHttpImpl extends BaseWxProfitSharingServiceImpl {
  @Override
  ...
  private HttpRequest buildHttpRequest(String url, String requestStr, boolean useKey) throws WxProfitSharingException {
  ...
  private String getResponseString(HttpResponse response) throws WxProfitSharingException {
    try {
      this.log.debug("【微信服务器响应头信息】：\n{}", response.toString(false));
    } catch (NullPointerException e) {
      throw new WxProfitSharingException("response.toString() 居然抛出空指针异常了", e);
    }

    String responseString = response.bodyText();

    if (StringUtils.isBlank(responseString)) {
      throw new WxProfitSharingException("响应信息为空");
    }
