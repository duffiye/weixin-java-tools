 ...
 */
@Data
public abstract class BaseWxProfitSharingRequest implements Serializable {
   ...
   */
  private void checkFields() throws WxProfitSharingException {
    //check required fields
    try {
      BeanUtils.checkRequiredFields(this);
    } catch (WxErrorException e) {
      throw new WxProfitSharingException(e.getError().getErrorMsg(), e);
    }
   ...
   * 检查约束情况
   */
  protected abstract void checkConstraints() throws WxProfitSharingException;
   ...
   */
  public void checkAndSign(WxProfitSharingConfig config) throws WxProfitSharingException {
    this.checkFields();
    ...
    if (StringUtils.isBlank(getSignType())) {
      if (config.getSignType() != null && !ALL_SIGN_TYPES.contains(config.getSignType())) {
        throw new WxProfitSharingException("非法的signType配置：" + config.getSignType() + "，请检查配置！");
      }
      this.setSignType(StringUtils.trimToNull(config.getSignType()));
    } else {
      if (!ALL_SIGN_TYPES.contains(this.getSignType())) {
        throw new WxProfitSharingException("非法的sign_type参数：" + this.getSignType());
      }
    }
