package com.github.binarywang.profitsharing.converter;

import com.github.binarywang.profitsharing.bean.notify.WxVehicleOrderNotifyCoupon;
import com.github.binarywang.profitsharing.bean.notify.WxVehicleOrderNotifyResult;
...
public class WxVehicleOrderNotifyResultConverter extends AbstractReflectionConverter {

  public WxVehicleOrderNotifyResultConverter(Mapper mapper, ReflectionProvider reflectionProvider) {
  ...
  public boolean canConvert(Class type) {
    return type.equals(WxVehicleOrderNotifyResult.class);
  }

  @Override
  public void marshal(Object original, HierarchicalStreamWriter writer, MarshallingContext context) {
    super.marshal(original, writer, context);
    WxVehicleOrderNotifyResult obj = (WxVehicleOrderNotifyResult) original;
    List<WxVehicleOrderNotifyCoupon> list = obj.getCouponList();
    if (list == null || list.size() == 0) {
      return;
    }
    for (int i = 0; i < list.size(); i++) {
      WxVehicleOrderNotifyCoupon coupon = list.get(i);
  ...
  public Object unmarshal(HierarchicalStreamReader reader, UnmarshallingContext context) {
    WxVehicleOrderNotifyResult obj = new WxVehicleOrderNotifyResult();
    ...
    List<WxVehicleOrderNotifyCoupon> coupons = new ArrayList<>(10);
  ...
  private void setFieldValue(UnmarshallingContext context, WxVehicleOrderNotifyResult obj, Field field) {
  ...
  private WxVehicleOrderNotifyCoupon getIndex(List<WxVehicleOrderNotifyCoupon> coupons, String nodeName) {
    ...
    if (index >= coupons.size() || coupons.get(index) == null) {
      coupons.add(index, new WxVehicleOrderNotifyCoupon());
    }
