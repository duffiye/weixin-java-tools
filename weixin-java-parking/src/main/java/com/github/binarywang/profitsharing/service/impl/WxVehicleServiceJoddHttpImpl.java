...
import com.github.binarywang.profitsharing.bean.WxVehicleApiData;
import com.github.binarywang.profitsharing.exception.WxVehicleException;
 ...
 */
public class WxVehicleServiceJoddHttpImpl extends BaseWxVehicleServiceImpl {
  @Override
  public byte[] postForBytes(String url, String requestStr, boolean useKey) throws WxVehicleException {
    try {
      ...
      wxApiData.set(new WxVehicleApiData(url, requestStr, responseString, null));
      return responseBytes;
    } catch (Exception e) {
      this.log.error("\n【请求地址】：{}\n【请求数据】：{}\n【异常信息】：{}", url, requestStr, e.getMessage());
      wxApiData.set(new WxVehicleApiData(url, requestStr, null, e.getMessage()));
      throw new WxVehicleException(e.getMessage(), e);
    }
  }

  @Override
  public String post(String url, String requestStr, boolean useKey) throws WxVehicleException {
    try {
      HttpRequest request = this.buildHttpRequest(url, requestStr, useKey);
  ...
  private HttpRequest buildHttpRequest(String url, String requestStr, boolean useKey) throws WxVehicleException {
  ...
  private String getResponseString(HttpResponse response) throws WxVehicleException {
    try {
      this.log.debug("【微信服务器响应头信息】：\n{}", response.toString(false));
    } catch (NullPointerException e) {
      throw new WxVehicleException("response.toString() 居然抛出空指针异常了", e);
    }

    String responseString = response.bodyText();

    if (StringUtils.isBlank(responseString)) {
      throw new WxVehicleException("响应信息为空");
    }
