...
import static com.github.binarywang.profitsharing.constant.WxVehicleConstants.SignType.ALL_SIGN_TYPES;
...
public abstract class BaseWxVehicleRequest implements Serializable {
   ...
   */
  private void checkFields() throws WxVehicleException {
    //check required fields
    try {
      BeanUtils.checkRequiredFields(this);
    } catch (WxErrorException e) {
      throw new WxVehicleException(e.getError().getErrorMsg(), e);
    }
   ...
   * 检查约束情况
   */
  protected abstract void checkConstraints() throws WxVehicleException;
   ...
   */
  public void checkAndSign(WxVehicleConfig config) throws WxVehicleException {
    this.checkFields();
    ...
    if (StringUtils.isBlank(getSignType())) {
      if (config.getSignType() != null && !ALL_SIGN_TYPES.contains(config.getSignType())) {
        throw new WxVehicleException("非法的signType配置：" + config.getSignType() + "，请检查配置！");
      }
      this.setSignType(StringUtils.trimToNull(config.getSignType()));
    } else {
      if (!ALL_SIGN_TYPES.contains(this.getSignType())) {
        throw new WxVehicleException("非法的sign_type参数：" + this.getSignType());
      }
    }
